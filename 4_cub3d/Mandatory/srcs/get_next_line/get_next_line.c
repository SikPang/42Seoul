...
/*   Updated: 2023/05/01 16:56:17 by kwsong           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ssize_t	read_buf(t_gnode *target, int fd)
{
	...
	while (1)
	{
		...
		if (byte <= 0)
			break ;
		temp = target->data;
		target->data = ft_strnjoin(target->data, buf, byte);
		if (target->data == 0)
		{
			target->data = temp;
			return (-2);
...
char	*get_next_line(int fd)
{
	...
	if (BUFFER_SIZE <= 0)
		return (0);
	target = gfind_node(&list, fd);
	if (target == 0)
		return (0);
	while (1)
	{
		result_len = ft_strchri(target->data, '\n');
		if (result_len > 0)
